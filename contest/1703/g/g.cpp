...
int coins[MAXN], C[MAXN], sum[MAXN];
int tmp[MAXN][34], KK, TT[MAXN][34];
int sv(int c, int k) {
    return TT[c][k];
//    while (k > 0 && c > 0) k--, c >>= 1;
//    return c;
...
int dp(int n, int k) {
    if (k >= 33) return dp(max(33ll, n), 33);
    if (tmp[n][k] >= -1e17) return tmp[n][k];
    if (k == 0) {
        return tmp[n][k] = sum[n] - n * KK;
    }
    int ans = dp(n-1, k-1) + sv(n, k);
    if (n >= k + 1) {
        ans = max(ans, dp(n-1, k) + sv(n, k) - KK);
    }
    return tmp[n][k] = ans;
}
...
void solve(int _case) {
    ...
    int N; cin >> N >> KK;
    sum[0] = 0;
    for (int i = 0; i <= N; i++) for (int k = 0; k < 34; k++) tmp[i][k] = -INF18, TT[i][k] = 0;
    for (int i = 1; i <= N; i++) {
        cin >> coins[i];
        sum[i] = sum[i-1] + coins[i];
        int w = coins[i];
        C[i] = 0;
        while (w > 0) TT[i][C[i]] = w, C[i]++, w >>= 1;
    }
    int ans = -INF18;
    for (int k = 0; k <= min(N, 34ll); k++) ans = max(ans, dp(N, k));
    cout << ans << endl;
}
