...
void solve(int _case) {
    ...
    if (n == 2) {
        ...
        ans[2][1] = ans[2][2] = b[2];
        ans[2][1]--;
        while (ans[2][1] < 0) ans[2][1] += 2;
        while (ans[2][1] >= n) ans[2][1] -= 2;
    } else {
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= n; j++) {
                ans[i][j] = ((i + j) * (i + j) + b[i]) % n;
                while (ans[i][j] >= n) ans[i][j] -= n;
                while (ans[i][j] < 0) ans[i][j] += n;
            }
        }
    }
