...
vector<int> grid[MAXN], tmp[MAXN], status;

void solve(int _case) {
    /** Code here for each Case */
    int n, m;
    cin >> n >> m;
    for (int i = 0; i < n; i++) grid[i].clear(), tmp[i].clear();
    ...
    for (int i = 0; i < n; i++) sort(tmp[i].begin(), tmp[i].end());

    int R = -1;
    for (int i = 0; i < n; i++) {
        status.clear();
        for (int j = 0; j < m; j++) if (tmp[i][j] != grid[i][j]) status.pb(j);
        if (status.size() == 0) continue;
        if (status.size() >= 3) { cout << -1 << endl; return; }
        R = i;
        break;
    }
    if (R == -1) { cout << "1 1" << endl; return; }
    for (int i = 0; i < n; i++) swap(grid[i][status[0]], grid[i][status[1]]);
    for (int i = 0; i < n; i++) {
        for (int j = 1; j < m; j++) {
            if (grid[i][j] < grid[i][j-1]) { cout << -1 << endl; return; }
        }
    }
    cout << (status[0]+1) << ' ' << (status[1]+1) << endl;
}
