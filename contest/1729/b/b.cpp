...
const string TABLE = "0abcdefghijklmnopqrstuvwxyz";

string toS(string num) {
    int a = num[0] - '0';
    if (num.length() == 2) a = 10 * a + num[1] - '0';
    return TABLE.substr(a, 1);
}
...
void solve(int _case) {
    ...
    string ans = "";
    for (int i = n - 1; i >= 0;) {
        if (buf[i] == '0') ans += toS(buf.substr(i-2, 2)), i-=3;
        else ans += toS(buf.substr(i, 1)), i--;
    }
