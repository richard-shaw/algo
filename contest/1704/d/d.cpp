...
void solve(int _case) {
    ...
    int K;
    for (int j = 0; j < m; j++) {
        for (int i = 1; i <= n; i++) if (c[i][j] % 2) odd[i]++; else even[i]++;
        map<int, vector<int>> mp; mp.clear();
        for (int i = 1; i <= n; i++) {
            int s = sv(i);
            if (mp.find(s) == mp.end()) {
        ...
        for (auto ite = mp.begin(); ite != mp.end(); ++ite) {
            if (ite->second.size() == 1) k = ite->second[0];
        }
        if (k == -1) continue;
        else { K = k; break;}
    }
