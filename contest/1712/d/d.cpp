...
void solve(int _case) {
    ...
    sort(arr + 1, arr + 1 + n);
    for (int i = 1; i <= k; i++) arr[i].first = 1e9;
    int M = 2 * arr[k + 1].first;
    for (int i = 1; i <= n; i++) A[arr[i].second] = arr[i].first;
    int K = -1;
    for (int i = 1; i < n; i++) A[i] = min(A[i], A[i+1]), K = max(K, A[i]);
    cout << min(K, M) << endl;
}
