...
set<int> co[MAXN];
int sv(int i, int t) {
    if (*co[i].begin() >= t) return *co[i].begin();
    if (*prev(co[i].end()) <= t) return *prev(co[i].end());
    auto ite = co[i].lower_bound(t);
    auto ans = ite;
    int tmp = abs(*ite - t);
    if (ite != co[i].begin()) {
        int k = abs(*prev(ite) - t);
        if (k < tmp) ans = prev(ite);
    }
    return *ans;
}
void solve(int _case) {
    ...
    int n, k; cin >> n >> k;
    for (int i = 1; i <= n; i++) cin >> arr[i], co[i].clear();
    for (int i = 1; i <= n; i++) {
        for (int p = 1; p <= k; p++) co[i].insert(arr[i] / p);
    }
    int ans = INF18;
    for (int t = 0; t <= 3000; t++) {
        int csm = sv(1, t), csM = csm;
        for (int i = 2; i <= n; i++) {
            int cs = sv(i, t);
            csm = min(csm, cs), csM = max(csM, cs);
        }
        ans = min(ans, csM - csm);
    }
    cout << ans << endl;
