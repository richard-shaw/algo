...
void solve(int _case) {
    ...
    k[n] = 0;
    for (int i = n - 1; i >= 1; i--) k[i] = k[i+1] - arr[i+1];
    if (k[1] != arr[1]) { NO; return; }
    bool ok = true, pre_zero = false;

    for (int i = 1; i < n; i++) {
        if (k[i] < 0) { ok = false; break; }
        if (k[i] == 0) pre_zero = true;
        else if (pre_zero) { ok = false; break; }
    }
    if (ok) YES;
    else NO;
}
