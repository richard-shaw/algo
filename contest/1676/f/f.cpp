...
void solve(int _case) {
    ...
    int cnt = 0;
    for (auto ite : mp) {
        if (ite.second >= K) arr[++cnt] = ite.first;
    }
    if (cnt == 0) { cout << -1 << endl; return; }
    sort(arr+1, arr+1+cnt);
    pair<int, int> ans = {arr[1], arr[1]};
    pair<int, int> pre = {arr[1], arr[1]};
    arr[0] = -12;
    for (int i = 1; i <= cnt; i++) {
        if (arr[i] == arr[i-1] + 1) pre.second++;
        else {
            if (pre.second-pre.first > ans.second-ans.first) ans = pre;
            pre = {arr[i], arr[i]};
        }
    }
    if (pre.second - pre.first > ans.second - ans.first) ans = pre;
