...
pair<int, int> dp(int i) {
    if (tmp[i].first != -1) return tmp[i];
    if (E[i].empty()) return {buf[i-1] == 'W', buf[i-1] == 'B'};
    pair<int, int> ans = {buf[i-1] == 'W', buf[i-1] == 'B'};
    for (int v : E[i]) {
        pair<int, int> ch = dp(v);
        ans.first += ch.first;
        ans.second += ch.second;
    }
    return tmp[i] = ans;
}
void solve(int _case) {
    ...
    int N;
    cin >> N;
    for (int i = 1; i <= N; i++) E[i].clear(), tmp[i].first = -1;
    for (int i = 2; i <= N; i++) {
        int p;
        cin >> p;
        E[p].pb(i);
