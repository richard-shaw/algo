...
int ST[MAXN][22], Log2[MAXN], mx[MAXN];
int query(int l, int r) {
    int s = Log2[r - l + 1];
    return max(ST[l][s], ST[r+1-(1<<s)][s]);
}
void solve(int _case) {
    /** Code here for each Case */
    for (int i = 0; i < MAXN; i++) ST[i][0] = 0, mx[i] = 0;
    Log2[0] = Log2[1] = 0;
    ...
    for (int c = 1; c <= C; c++) {
        if (mx[c] == 0) continue;
        for (int tc = c, tn = 1; tc <= C; tc += c, tn++) {
            ST[tc][0] = max(ST[tc][0], tn * mx[c]);
        }
    }
    ...
    for (int i = 1; i <= M; i++) {
        int D, H;
        cin >> D >> H;
        int L = 1, R = C, Target = D * H + 1;
        if (query(L, R) < Target) cout << -1;
        else {
            while (L < R) {
                int Mid = (L + R) >> 1;
                if (query(L, Mid) >= Target) R = Mid;
                else L = Mid + 1;
            }
            cout << L;
        }
