...
int find_card(int l, int r, int h) {
    if (l > r) return -1;
    if (h > card[r]._hash || h < card[l]._hash) return -1;
    while (l < r) {
        int m = (l + r) >> 1;
        if (h == card[m]._hash) return m;
        if (h < card[m]._hash) r = m;
        else l = m + 1;
    }
    return card[l]._hash == h ? l : -1;
}
...
void solve(int _case) {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        card[i]._hash = 0;
        for (int j = 1; j <= k; j++) {
            cin >> card[i].feature[j];
            card[i]._hash = 3 * card[i]._hash + card[i].feature[j];
        }
    }
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = i + 1; j <= n; j++) {
            ...
            for (int t = 1; t <= k; t++) {
                int f;
                if (card[i].feature[t] == card[j].feature[t]) f = card[i].feature[t];
                else f = 3 - card[i].feature[t] - card[j].feature[t];
                _h = 3 * _h + f;
            }
            int id = find_card(j + 1, n, _h);
            if (id == -1) continue;
            cnt[i]++;
            cnt[j]++;
            cnt[id]++;
    ...
    int ans = 0;
    for (int i = 1; i <= n; i++) ans += (cnt[i] - 1) * cnt[i] / 2;
    cout << ans << endl;
}
