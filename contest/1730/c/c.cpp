...
int dig[MAXN], cnt[MAXN];
void solve(int _case) {
    /** Code here for each Case */
    string buf; cin >> buf;
    int n = buf.length();
    for (int i = 1; i <= n; i++) dig[i] = buf[i-1] - '0', cnt[i] = 0;
    ...
    if (pr == 10) cout << buf << endl;
    else {
        for (int i = 1; i <= n; i++) if (cnt[i]) dig[i] += (dig[i] == 9 ? 0 : 1);
        sort(dig + 1, dig + 1 + n);
        for (int i = 1; i <= n; i++) cout << dig[i];
        cout << endl;
    }
}
