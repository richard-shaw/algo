...
const int MAXN = 1001;
bitset<6000> dp[MAXN][2];
int arr[MAXN][MAXN];
inline int get_idx(int sum) { return sum  + 2999; }
void solve(int _case) {
    /** Code here for each Case */
    int n, m;
    cin >> n >> m;
    for (int i = 1; i <= n; i++) for (int j = 1; j <= m; j++) cin >> arr[i][j];
    for (int i = 1; i <= n; i++) dp[i][0].reset(), dp[i][1].reset();

    int cur = 0;
    dp[1][cur].set(get_idx(arr[1][1]));
    for (int i = 2; i <= n; i++) dp[i][cur] = (arr[i][1] == 1) ? (dp[i-1][cur] << 1) : (dp[i-1][cur] >> 1);
    for (int j = 2; j <= m; j++) {
        cur = 1 - cur;
        dp[1][cur] = (arr[1][j] == 1) ? (dp[1][1-cur] << 1) : (dp[1][1-cur] >> 1);
        for (int i = 2; i <= n; i++) {
            dp[i][cur] = dp[i-1][cur] | dp[i][1-cur];
            if (arr[i][j] == 1) dp[i][cur] <<= 1;
            else dp[i][cur] >>= 1;
        }
    }
    cout << (dp[n][cur][get_idx(0)] ? "YES" : "NO") << endl;
}
