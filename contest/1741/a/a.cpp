...
pair<int, int> toI(string &s) {
    int l = s.length();
    if (s[l-1] == 'M') return { 0, 0 };
    else if (s[l-1] == 'S') return { l-1, -1};
    else return {l-1, 1};
}
char cmp(string &s1, string &s2) {
    pair<int, int> a1 = toI(s1), a2 = toI(s2);
    if (a1.second > a2.second) return '>';
    else if (a1.second < a2.second) return '<';
    else if (a1.first * a1.second < a2.first * a2.second) return '<';
    else if (a1.first * a1.second > a2.first * a2.second) return '>';
    else return '=';
}
